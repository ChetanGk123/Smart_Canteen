...
export class ManagaeCanteenComponent implements OnInit {
    ...
    commonForm: FormGroup = new FormGroup({
        ...
        canteen_name: new FormControl(this.config?.data?.canteen_name ?? '', [
            Validators.required,
        ]),
        address: new FormControl(this.config?.data?.address ?? '', [
            Validators.required,
        ]),
        registered_on: new FormControl(this.config?.data?.registered_on ?? '', [
            Validators.required,
        ]),
        ...
        if (this.config?.data?.registered_on) {
            ...
            this.commonForm.controls.registered_on.setValue(
                this.datepipe.transform(
                    new Date(
                        Number(registered_on[2]) +
                            '/' +
                            Number(registered_on[1]) +
                            '/' +
                            Number(registered_on[0])
                    ),
                    'yyyy-MM-dd'
                )
            );
