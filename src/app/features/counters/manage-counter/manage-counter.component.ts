...
export class ManageCounterComponent implements OnInit {
    ...
    commonForm: FormGroup = new FormGroup({
        counter_id: new FormControl(
            this.config?.data?.counter_id ?? this.config?.data?.id ?? ''
        ),
        role_id: new FormControl(''),
        school_name: new FormControl(this.config?.data?.school_name ?? '', [
            Validators.required,
        ]),
        counter_name: new FormControl(this.config?.data?.counter_name ?? '', [
            Validators.required,
        ]),
        counter_address: new FormControl(
            this.config?.data?.counter_address ?? '',
            [Validators.required]
        ),
        contact_person: new FormControl(
            this.config?.data?.contact_person ?? '',
            [Validators.required]
        ),
        contact_number: new FormControl(
            this.config?.data?.contact_number ?? '',
            [Validators.required]
        ),
